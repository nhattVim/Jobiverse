...
const Security = () => {
  ...
  const fetchHasPassword = useCallback(async () => {
    try {
      ...
      toast.error(`Không thể kiểm tra trạng thái mật khẩu: ${err.message || ''}`)
    }
  }, [])
  ...
  }, [fetchHasPassword, user.phoneNumber])
  ...
  const handlePhoneSubmit = async e => {
    ...
    if (!/^\d{10,11}$/.test(phone)) {
      toast.error('Số điện thoại không hợp lệ') && setPhone(user.phoneNumber || '')
      return
    }
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="flex items-start gap-16 px-6 py-20 mx-auto max-w-7xl">
        <div className="flex-shrink-0 w-1/4"><Sidebar /></div>
