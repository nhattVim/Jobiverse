...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
        => optionsBuilder.UseMySQL("server=mysql-ed145c2-hochitrung08012004-6685.j.aivencloud.com;port=22012;database=Jobiverse_2;user=avnadmin;password=AVNS_23e2BqV9YnTCreBLUIT;");
            ...
            entity.HasMany(d => d.Majors).WithMany(p => p.Projects)
                .UsingEntity<Dictionary<string, object>>(
                    ...
                    r => r.HasOne<Major>().WithMany()
                        ...
                        .OnDelete(DeleteBehavior.ClientSetNull)
                        .HasConstraintName("ProjectMajors_ibfk_2"),
                    l => l.HasOne<Project>().WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.ClientSetNull)
                    ...
                    j =>
                    {
                        j.HasKey("ProjectId", "MajorId").HasName("PRIMARY");
                        j.ToTable("ProjectMajors");
                        j.HasIndex(new[] { "MajorId" }, "MajorID");
                        j.IndexerProperty<string>("ProjectId").HasColumnName("ProjectID");
                        j.IndexerProperty<string>("MajorId").HasColumnName("MajorID");
                    });

            entity.HasMany(d => d.Specializations).WithMany(p => p.Projects)
                .UsingEntity<Dictionary<string, object>>(
                    "ProjectSpecialization",
                    r => r.HasOne<Specialization>().WithMany()
                        .HasForeignKey("SpecializationId")
                        .HasConstraintName("ProjectSpecializations_ibfk_2"),
                    l => l.HasOne<Project>().WithMany()
                        .HasForeignKey("ProjectId")
                        .HasConstraintName("ProjectSpecializations_ibfk_1"),
                    j =>
                    {
                        j.HasKey("ProjectId", "SpecializationId").HasName("PRIMARY");
                        j.ToTable("ProjectSpecializations");
                        j.HasIndex(new[] { "SpecializationId" }, "SpecializationID");
                        j.IndexerProperty<string>("ProjectId").HasColumnName("ProjectID");
